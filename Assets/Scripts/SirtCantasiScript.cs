...
public class SirtCantasiScript : MonoBehaviour
{
    ...
    private void CantayiHizala()//Reklam Icın Duzenlendi
    {
        for (int i = 0; i < _sirtCantasiObject.transform.childCount; i++)
        {
            _sirtCantasiObject.transform.GetChild(i).transform.position = _moneyStackNoktalari[i].transform.position;//Yerlesme Noktalari Ile Degistir
        }
